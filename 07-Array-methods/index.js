//.from() and .of()
//both of them are not on the prototype
//but they are on the Array itself.
//Array.from() turn something Arrayish into Array
//ex. NodeList
//array.from() takes a second argument a map function to modify data
...
//another use case is to convert arguments obj into array
...
//Array.of() going to create an array from all the arguents passed to it

const ages = Array.of(30,32,44,65,22,19);
...
//.find() common use case is when you have data from a source having objects and you need to find something in the object. it will return actual element and only one return, for multiple use filter

//.findIndex() is helpful when you have the item and want to know where in the array it's placed.
...
const newAges = [32,15,18,82,77,33];
console.log(newAges);
///.some() will look for some of the items meeting the condition and return true as soon as it finds one item which is true against the condition

//is there atleast one adult in the group of ages?
const adultPreset = newAges.some(age => age >= 18);
console.log(adultPreset);

//.every() will look for every element should match the condition then it will return true;
//is everyone old enough to drink?
const allAllowed = newAges.every(age => age >= 18);
